...
use crate::thesqisign::{
    sqisign_lvl3_broadwell_sqisign_keypair, sqisign_lvl3_broadwell_sqisign_sign,
    sqisign_lvl3_broadwell_sqisign_verify,
};

/// Empty type, contains the SQISign lvl3 implementations.
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Debug)]
pub struct Lvl3;

impl InternalLevel for Lvl3 {
    unsafe fn keypair(public_key: *mut u8, private_key: *mut u8) -> i32 {
        unsafe { sqisign_lvl3_broadwell_sqisign_keypair(public_key, private_key) }
    }
    unsafe fn sign(
        ...
        unsafe {
            sqisign_lvl3_broadwell_sqisign_sign(
                signed_message,
                signed_message_len,
                message,
                message_len,
...
impl Level for Lvl3 {
    const PUBLIC_KEY_BYTES: usize = 97;
    const PRIVATE_KEY_BYTES: usize = 529;
    const SIGNATURE_BYTES: usize = 224;
